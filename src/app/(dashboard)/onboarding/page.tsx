...
import { Moon, Star, Eye, Sun, Feather, Leaf, Sparkles, Heart, Brain, Music, Hand, Flame } from 'lucide-react'
    ...
    <div className="max-w-2xl mx-auto py-10">
      ...
      {step === 0 && (
        <div className="space-y-5">
          ...
          <div>
            <label className="block text-sm font-medium text-zinc-300 mb-2">Descreva seu trabalho em poucas palavras</label>
            <textarea
              value={brandDescription}
              onChange={e => setBrandDescription(e.target.value)}
              placeholder="Ex: Sou astróloga e taróloga, atendo mulheres em processo de autoconhecimento. Trabalho com mapa astral, tarot terapêutico e rituais de lua..."
              rows={3}
            ...
            />
            <p className="text-xs text-zinc-500 mt-1">Isso ajuda a IA a entender sua essência e gerar conteúdo mais autêntico</p>
          </div>
      ...
      {/* Step 1: Specialties (multi-select) */}
      {step === 1 && (
        <div className="space-y-5">
          <div>
            <label className="block text-sm font-medium text-zinc-300 mb-1">Quais são suas especialidades?</label>
          ...
          <div>
            <label className="block text-sm font-medium text-zinc-300 mb-2">Outra especialidade? (opcional)</label>
            <input
              type="text"
              value={specialtiesOther}
              onChange={e => setSpecialtiesOther(e.target.value)}
              placeholder="Ex: Radiestesia, ThetaHealing, Florais de Bach..."
              className="w-full px-4 py-3 rounded-lg bg-zinc-900 border border-zinc-800 text-white placeholder:text-zinc-600 focus:outline-none focus:border-violet-500"
            />
          </div>
          {specialties.length === 0 && (
            <p className="text-xs text-amber-400">Selecione pelo menos uma especialidade</p>
          )}
      ...
      {step === 2 && (
        <div className="space-y-5">
          <div>
            <label className="block text-sm font-medium text-zinc-300 mb-2">Quem é seu público-alvo?</label>
            <textarea
              value={targetAudience}
              onChange={e => setTargetAudience(e.target.value)}
              placeholder="Ex: Mulheres de 25-45 anos em busca de autoconhecimento, que acompanham astrologia e espiritualidade no Instagram..."
              rows={3}
              className="w-full px-4 py-3 rounded-lg bg-zinc-900 border border-zinc-800 text-white placeholder:text-zinc-600 focus:outline-none focus:border-violet-500 resize-none"
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-zinc-300 mb-1">Objetivos com o conteúdo</label>
            <p className="text-xs text-zinc-500 mb-3">Selecione todos que se aplicam</p>
            <div className="grid grid-cols-2 gap-2">
              {CONTENT_GOALS.map(({ id, label }) => (
                <button
                  key={id}
                  onClick={() => toggleGoal(id)}
                  className={`px-3 py-2.5 rounded-lg border text-left text-sm transition ${
                    contentGoals.includes(id)
                      ? 'border-violet-500 bg-violet-500/10 text-violet-300'
                      : 'border-zinc-800 text-zinc-400 hover:border-zinc-700'
                  }`}
                >
      ...
      {step === 3 && (
        <div className="space-y-5">
          <div>
            <label className="block text-sm font-medium text-zinc-300 mb-1">Cores da sua marca</label>
            <p className="text-xs text-zinc-500 mb-4">Selecione as paletas que mais combinam com sua identidade</p>
              ...
              {COLOR_PALETTES.map(({ id, label, colors }) => (
                ...
                >
                  {colors.length > 0 ? (
                    ...
                    <div className="w-5 h-5 rounded-full border-2 border-dashed border-zinc-600" />
                  )}
          ...
          {brandColors.includes('custom') && (
            <div>
              <label className="block text-sm font-medium text-zinc-300 mb-2">Quais são suas cores? (hex ou nomes)</label>
              <input
                type="text"
                value={customColors}
                onChange={e => setCustomColors(e.target.value)}
                placeholder="Ex: #7c3aed, dourado, verde escuro, rosa blush"
                className="w-full px-4 py-3 rounded-lg bg-zinc-900 border border-zinc-800 text-white placeholder:text-zinc-600 focus:outline-none focus:border-violet-500"
              />
            </div>
          )}
      ...
      {step === 4 && (
        <div className="space-y-5">
          <div>
            <label className="block text-sm font-medium text-zinc-300 mb-1">Estilo visual das imagens</label>
            <p className="text-xs text-zinc-500 mb-4">Como você quer que as imagens geradas pareçam?</p>
            <div className="space-y-2">
      ...
      {step === 5 && (
        <div className="space-y-5">
          <div>
            ...
            <p className="text-xs text-zinc-500 mb-4">Como você se comunica com sua audiência?</p>
            <div className="space-y-2">
